...
class JobsClient(dbclient):
  ...
  def find_empty_jobs(self):
    ...
    empty_jobs = filter(lambda x: self.is_all_empty(x), jobs)
    # find the creators of this job to see how often these users create empty jobs
    creators_untitled = map(lambda x: x.get('creator_user_name', 'unknown'), untilted_jobs)
    creators_empty = map(lambda x: x.get('creator_user_name', 'unknown'), empty_jobs)
    # convert into a set to remove duplicates from the list
    empty_job_ids = map(lambda x: {'job_id': x['job_id'], 'creator': x.get('creator_user_name', 'unknown')}, empty_jobs + untilted_jobs)
  ...
  def get_scheduled_jobs(self):
    ...
    for x in scheduled_jobs:
      ...
      y['created_time'] = datetime.datetime.fromtimestamp(x['created_time'] / 1000.0).strftime('%Y-%m-%d %H:%M:%S.%f')
